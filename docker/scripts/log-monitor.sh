...
classify_issue() {
    ...
    if echo "$line" | grep -qiE "Received connection for vanilla player|Received context for farmhand .* running SMAPI .* with 0 mods"; then
        echo "CLIENT_JOINED_WITHOUT_MODS"; return 0
    fi
