...
const RULES = [
  ...
  {
    code: 'DUPLICATE_MOD_COPY',
    severity: 'error',
    category: 'mod',
    title: 'Duplicate mod copies are installed',
    cause: 'SMAPI found the same mod in more than one folder, often caused by a zip that contains an extra Mods/ folder.',
    action: 'Remove nested Mods/Mods copies or run the latest panel update so the startup repair can back them up and delete the nested folder.',
    pattern: /multiple copies of this mod installed|delete these folders and reinstall the mod/i,
  },
  {
    code: 'SMAPI_TOO_OLD_FOR_MOD',
    severity: 'error',
    category: 'mod',
    title: 'SMAPI is too old for an installed mod',
    cause: 'One or more mods require a newer SMAPI version than the server is currently running.',
    action: 'Update the panel release, rebuild the server image, and let startup reinstall the bundled SMAPI.',
