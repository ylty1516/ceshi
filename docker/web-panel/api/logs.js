...
const configuredMaxLines = parseInt(process.env.PANEL_LOG_MAX_LINES || '600', 10);
const configuredDefaultLines = parseInt(process.env.PANEL_LOG_DEFAULT_LINES || '200', 10);
const MAX_LINES = Number.isFinite(configuredMaxLines) && configuredMaxLines > 0 ? configuredMaxLines : 600;
const DEFAULT_LINES = Number.isFinite(configuredDefaultLines) && configuredDefaultLines > 0 ? configuredDefaultLines : 200;
const configuredTailBytes = parseInt(process.env.PANEL_LOG_TAIL_BYTES || String(512 * 1024), 10);
...
function subscribeLogs(ws, filter) {
  ...
  const watcher = fs.watch(logPath, (eventType) => {
    ...
    try {
      ...
      const streamStart = Math.max(fileSize, stat.size - MAX_TAIL_BYTES);
