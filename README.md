...
这也是“服务器明明有空闲小屋，但玩家看到没有空闲位置”的常见原因之一：服务器已经按原生 `Co-op -> Host` 发出了 farmhand 席位列表，但客户端缺少内容 Mod、版本不同或依赖不同，可能在选择 farmhand / 载入世界前断开或误报无空位。遇到这种情况先不要只改小屋数量，先让玩家安装面板生成的整包并核对指纹。

如果服务器日志里出现 `Received connection for vanilla player`，或者 `Received context for farmhand ... running SMAPI ... with 0 mods`，基本可以判断玩家没有用正确的 SMAPI/Mods 目录进入。新版面板会在“加入握手”和“诊断”里直接提示“客户端未用 SMAPI”“客户端未加载 Mod”或“客户端 Mod 不完整”，用于区分客户端 Mod 问题和真正的小屋/席位问题。

但它不是唯一原因。诊断页现在会显示 `Save slot and cabin audit`，用于排查另外几类会造成同样提示的问题：实际选中的存档不是你以为的那个、`runtime.env`/`.selected_save`/ServerAutoLoad 配置互相冲突、`SAVE_NAME` 被错误地带上引号、`playerLimit` 没有写到 8、`enableFarmhandCreation` 被关闭、存档里的小屋数量不足，或者旧 farmhand 记录已经占满可用席位。遇到“有 7 个小屋但仍然无空位”时，先看这个审计项和“加入握手”项，再决定是修存档、修配置还是让玩家重新安装本地 Mod 包。
...
新版下载包会包含 `ylty-client-mod-lock.json`。它记录服务器要求的客户端 Mod、版本、每个 Mod 的 SHA256 和整包指纹。公开下载页也会显示同一个整包指纹；如果玩家本地不是这套包，就不能认为“服务器和本地 Mod 相同”。
...
这个页面不需要管理员登录，只能下载玩家 Mod 包、单个玩家必需 Mod 和查看 `mod-manifest.json` 校验清单。清单会显示每个 Mod 的名称、版本、文件大小、更新时间、SHA256 和整包指纹，方便玩家核对本地安装是否一致。
